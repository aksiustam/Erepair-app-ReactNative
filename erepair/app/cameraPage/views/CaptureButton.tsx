...
import { CAPTURE_BUTTON_SIZE, SCREEN_HEIGHT, SCREEN_WIDTH } from '../CameraConstant'
...
interface Props extends ViewProps {
  ...
  onMediaCaptured: (media: PhotoFile | VideoFile, type: 'image' | 'video') => void
...
const _CaptureButton: React.FC<Props> = ({
  ...
  const circleWidth = CAPTURE_BUTTON_SIZE + 10
  const radius = (CAPTURE_BUTTON_SIZE + 5) / 2;
  ...
  const startAnimation = () => {
    ...
    strokeDashoffset.value = withTiming(0, { duration:60000 ,easing: Easing.linear});
  };
        ...
        case State.CANCELLED: {
          ...
          try {
            ...
            if (diff < START_RECORDING_DELAY) {
              ...
              resetAnimation()
              clearTimeout(recordingTimeout) // Clear the timeout if recording is stopped early
              await stopRecording()
            }
          } finally {
            setTimeout(() => {
