...
const YazilimsalPage = ({ pctamir, setPageContent, sorundata }) => {
  ...
  const onSubmit = async (data) => {
    if (checkmedia) {
      ...
      await Alert.alert(
        "Başarılı!",
        "Cihazınızın Yazılımsal Sorunu Başarıyla Kaydedildi."
      );
      if (bucket1.sorun.some((item) => item.check === false)) {
        await Alert.alert(
          "Devam Ediniz!",
      ...
      <View className="h-full mx-2 ">
        <View className="px-6 py-1 rounded-full bg-white my-3">
          <Text className="text-2xl font-gmedium text-center">
            Yazılımsal Sorunu Olan Cihazın Bilgilerini Girelim!
          </Text>
        </View>
