...
const HataPage = ({ pctamir, setPageContent, sorundata }) => {
  ...
  const onSubmit = async (data) => {
    if (checkmedia) {
      ...
      await Alert.alert(
        "Başarılı!",
        "Cihazınızın Hata Alma Sorunu Başarıyla Kaydedildi."
      );
      if (bucket1.sorun.some((item) => item.check === false)) {
        await Alert.alert(
          "Devam Ediniz!",
      ...
      <View className="h-full mx-2 ">
        <View className="px-6 py-1 rounded-full bg-white my-3">
          <Text className="text-2xl font-gmedium text-center">
            Hata Alan Cihazın Bilgilerini Girelim!
          </Text>
        </View>
