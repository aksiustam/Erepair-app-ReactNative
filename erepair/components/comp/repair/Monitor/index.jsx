...
import { MonitorMarkaData } from "../../../../constant/MonitorMarka";
...
const MonitorPage = () => {
  ...
  const filterData =
    search === ""
      ? MonitorMarkaData
      : MonitorMarkaData.filter((item) =>
          item.name.toLowerCase().includes(search.toLowerCase())
        );
  return (
    <View className="flex-1 flex-col items-center justify-center ">
      <View className="flex justify-center items-center mt-6 mb-3 ">
        <Text className="text-4xl font-gregularitalic text-slate-50">
      ...
      <View className="w-full h-full mx-2">
        ...
        >
          {filterData?.map((item) => (
            <TouchableOpacity
              key={item.id}
              className="w-[40%] border-2 border-primary-500 rounded-xl flex items-center justify-center"
            ...
            >
              ...
              <Text className="mt-1 text-3xl font-gbolditalic text-slate-100">
                {item.name}
              </Text>
            </TouchableOpacity>
          ))}
